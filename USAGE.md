...
1. 当用户发送"活跃市值"时，插件会自动从indexes API获取股市数据并回复给用户。
2. 当用户发送"异动 [股票代码]"时，插件会自动从analyze API获取指定股票的异动分析数据并回复给用户。
3. 当用户发送"涨停看板"时，插件会自动从limit_up API下载图片并回复给用户。
4. 当用户发送"选股 [策略名称]"时，插件会根据指定策略从dyq_select API获取选股结果并回复给用户。
