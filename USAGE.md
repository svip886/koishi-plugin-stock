...
## 插件功能
`koishi-plugin-stock` 是一个Koishi插件，包含多个主要功能：
1. 当用户发送"活跃市值"时，插件会自动从`indexes`API获取股市数据并回复给用户。
2. 当用户发送"异动 [股票代码]"时，插件会自动从`analyze`API获取指定股票的异动分析数据并回复给用户。
3. 当用户发送"涨停看板"时，插件会自动从`limit_up`API下载图片并回复给用户。
4. 当用户发送"跌停看板"时，插件会自动从`limit_down`API下载图片并回复给用户。
5. 当用户发送"选股 [策略名称]"时，插件会根据指定策略从`dyq_select`API获取选股结果并回复给用户。
6. 当用户发送"骑"时，插件会返回本地图片`images/qi.jpeg`。

此外，插件还支持为每个指令单独设置黑名单，管理员可以限制特定用户或频道使用特定功能。
